...
export class ListSowComponent implements OnInit {
  ...
  deleteItem(i: number, id: number, sow_id: number, sow_name: string, sow_budget: number, sow_status: string, sow_startdate: string,sow_enddate: string, client_id:number,currency_type:string,remarks:string) {
    this.index = i;
    this.id = id;
  ...
  connect(): Observable<Issue[]> {
    ...
    return Observable.merge(...displayDataChanges).map(() => {
      ...
      this.filteredData = this._exampleDatabase.data.slice().filter((issue: Issue) => {
        const searchStr = (issue.sow_id + issue.sow_name + issue.sow_budget + issue.sow_status +
          issue.sow_startdate + issue.sow_enddate + issue.client_id + 
          issue.currency_type + issue.remarks).toString().toLowerCase();
